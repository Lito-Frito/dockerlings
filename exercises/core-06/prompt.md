...
3.  **Use the `EXPOSE`** instruction with the `$PORT` environment variable.
4.  **Set the default `CMD`** to run the Flask application. The application (`app.py`) is already written to read the `$PORT` environment variable.
